  ...
  raiseEvent: function (result) {
    ...
    return this.checkIfNew(result)
      ...
      .then(function (isNew) {
        ...
        var eventName = result.connectorKey + ((result.result.status && result.result.status === 'DELETED') ? ':deleted:' : (isNew ? ':new:' : ':modified:')) + result.singularEndpointName.toLowerCase();
        classLogger.info({
          eventName: eventName,
          subscription: this.context.subscription._id,
          application: this.context.application._id
