  ...
  assertCanPoll: function () {
    return BBPromise.try(function () {
      ...
      if (lastPolled > moment().subtract(frequency, 'minutes').utc().format()) {
        ...
        this.context.subscription.delayTill(lastPolled.add(frequency, 'minutes').toDate());
  ...
  handleResults: function (results, endpoint, singularEndpointName, _lastPoll, timeNow) {
    ...
    return BBPromise.settle(_.map(mappedResults, _.bind(this.raiseEvent, this)))
      ...
      .then(function (promises) {
        ...
        var endpoint = this.context.subscription.get(endpoint);
        endpoint.lastPolled = lastPolled;
        this.context.subscription.set(endpoint, endpoint);
        return promises;
      }).catch(function (err) {
        classLogger.error({
          err: {
