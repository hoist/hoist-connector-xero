...
var fs = require('fs');
var auth = require('./generatePublicPrivateKeyPair');
...
XeroConnector.defaultSettings = function(authType) {
  if (authType === 'Private') {
    return auth.generatePublicPrivateKeyPair({ stamp: Date.now() })
      .then(function(keys) {
        return {
  ...
  return new BBPromise(function(resolve, reject) { resolve(true) });
};
  ...
  if (this.settings.authType === 'Public') {
    if (bounce.get('RequestToken')) {
      ...
      return this.swapRequestToken(bounce.get('RequestToken'), bounce.get('RequestTokenSecret'), bounce.query.oauth_verifier)
        .spread(function (accessToken, accessTokenSecret) {
...
XeroConnector.prototype.request = function request(method, path, data) {
  ...
  return this.auth._performSecureRequestAsync(this.settings.accessKey, this.settings.accessSecret, method, baseUrl + path, null, data, contentType)
