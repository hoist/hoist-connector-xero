...
function XeroConnector(settings) {
  ...
  if (settings.authType === 'Partner') {
    this.auth._createClient = _.bind(function (port, hostname, method, path, headers, sslEnabled) {
      ...
      var certificates = certificateStore.get('XERO');
      var options = {
        host: hostname,
        port: port,
        path: path,
    ...
    return this.authorization.set('AccessToken', accessToken)
      .then(function () {
        ...
        if (auth_headers && auth_headers.oauth_session_handle) {
          var tokenExpiresAt = moment().add(parseInt(auth_headers.oauth_expires_in));
...
XeroConnector.prototype.request = function request(method, path, data) {
  ...
  return this.auth._performSecureRequestAsync(this.settings.accessKey, this.settings.accessSecret, method, this.authConfig.url + path, null, data, contentType)
