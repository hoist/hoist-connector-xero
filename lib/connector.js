...
XeroConnector.prototype.request = function request(method, requestPath, data, extraHeader) {
  ...
  var url = this.authConfig.url;
  if (this.settings.runscopeBucket && this.settings.authType !== 'Partner') {
    //redirect to runscope
    url = url.replace('.', '-') + '-' + this.settings.runscopeBucket + ".runscope.net";
  }
  url = url + requestPath;
  logger.info({
    method: method,
  ...
  return this.auth._performSecureRequestAsync(this.settings.accessKey, this.settings.accessSecret, method, url, null, data, contentType, extraHeader)
